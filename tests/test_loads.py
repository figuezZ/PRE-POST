...
def test_section_effects_match_hand_calculation():
    result = simply_supported_uniform_load_at_section(
        uniform_load_n_m=8000.0,
        span_m=10.0,
        position_from_left_m=2.5,
    )

    assert result.left_reaction_n == pytest.approx(40_000.0)
    assert result.shear_n == pytest.approx(20_000.0)
    assert result.moment_n_m == pytest.approx(75_000.0)


@pytest.mark.parametrize("position", [-0.001, 10.001])
def test_section_effects_reject_position_outside_span(position):
    with pytest.raises(ValueError, match="dentro de la luz"):
        simply_supported_uniform_load_at_section(1.0, 10.0, position)
